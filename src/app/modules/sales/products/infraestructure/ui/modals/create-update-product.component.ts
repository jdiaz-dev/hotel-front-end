...
import { ProductData } from '../../../../../../shared/interfaces/product/get-product-response';
...
export class CreateUpdateProductComponent implements OnInit {
  ...
  paramsToUpdateProduct() {
    this.product =
      this.data !== null
        ? new ProductModel(this.data.code, this.data.name, this.data.brand, this.data.details, this.data.price)
        : this.product;
  ...
  createProduct() {
    this.productsService.createProduct(this.productData.value).subscribe(
      (response: any) => {
  ...
  updateProduct() {
    this.productsService.updateProduct(this.productData.value, this.productId).subscribe(
      (response: any) => {
        console.log(response);
      },
      (error) => {
