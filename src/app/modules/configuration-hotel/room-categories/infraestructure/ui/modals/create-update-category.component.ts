...
export class CreateUpdateCategoryComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.roomCategoryData = this.formBuilder.group({
            nameCategory: [
                '',
                [Validators.required, Validators.maxLength(50), Validators.pattern(REG_EXP.alphanumeric)],
            ],
    ...
    saveHotelLevel(form: any) {
        const data = new RoomCategoryModel(this.roomCategoryData.value.nameCategory, this.roomCategoryData.value.price);
