...
export class CreateUpdateRoomComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.roomData = this.formBuilder.group({
            ...
            details: ['', [Validators.required, Validators.maxLength(200), Validators.pattern(REG_EXP.alphanumeric)]],
    ...
    private createRoom(data: RoomModel) {
        this.roomsPersistenceService.createRoom(data).subscribe(
            (response: any) => {
    ...
    private updateRoom(data: RoomModel) {
        this.roomsPersistenceService.updateRoom(data, this.levelId, this.categoryId, this.roomId).subscribe(
            (response: any) => {
                console.log(response);
                this.ngOnInit();
            },
